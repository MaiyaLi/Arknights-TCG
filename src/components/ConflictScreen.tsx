...
export default function ConflictScreen({ userProfile, onUpdateProfile, onBack, onMatchEnd }: ConflictScreenProps) {
  ...
  const startMatch = (mId: string, amIHost: boolean) => {
    ...
    const kernel = new BattleKernel(
      (w) => {
          if (amIHost) {
              matchChannel.send({ type: 'broadcast', event: 'game_over', payload: { winner: w } });
              syncMatchState(matchChannel);
          }
      },
      () => {},
      (unit, reason) => {
          if (amIHost && reason !== 'SCORED_GOAL') {
              matchChannel.send({ type: 'broadcast', event: 'unit_removed', payload: { owner: unit.owner, opId: unit.id } });
          }
      },
      (p) => {
          setPhase(p);
    ...
    matchChannel
      ...
      .on('broadcast', { event: 'match_sync' }, ({ payload }) => {
        if (!amIHost && kernelRef.current) {
          ...
          setUiState({
            ...
            playerDP: Math.floor(payload.playerDP),
            opponentDP: Math.floor(payload.opponentDP)
          });
      ...
      .on('broadcast', { event: 'unit_removed' }, ({ payload }) => {
          ...
          if (isMe) {
              const op = ALL_ASSETS.find(a => a.id === payload.opId);
              if (op) {
                  let cooldown = op.class === 'Specialist' ? 1 : 4;
  ...
  const render = React.useCallback(() => {
    ...
    for (let i = 0; i <= 8; i++) {
        const pS = project(i * (5 / 8) - 1.5, -0.5); const pE = project(i * (5 / 8) - 1.5, 6.5);
        ctx.beginPath(); ctx.moveTo(pS.x, pS.y); ctx.lineTo(pE.x, pE.y); ctx.stroke();
    }
    ...
    for (let r = 0; r < 7; r++) {
      for (let l = 0; l < 3; l++) {
        ...
        ctx.strokeStyle = isSelected ? 'rgba(0, 255, 231, 1)' : 'rgba(0, 152, 217, 0.15)';
    ...
    kernelRef.current?.units.forEach(u => {
      ...
      const basePos = project(displayLane, displayRow);
      const spriteImg = spriteImages.current[`${u.id}_${view}`];

      if (spriteImg && spriteImg.complete) {
        ctx.save(); ctx.shadowBlur = 10; ctx.shadowColor = mainColor + '44';
  ...
  useEffect(() => {
    let animId: number;
    const loop = () => {
      if (isHost && kernelRef.current && phase === 'ACTION' && !isPaused) {
  ...
  const handleDragStart = (op: Operator, index: number, e: any) => {
    ...
    setDragPos({ x: e.clientX || e.touches[0].clientX, y: e.clientY || e.touches[0].clientY });
  };
  ...
  const handleDragMove = (e: any) => {
    ...
    if (rect) {
      const ux = (x - rect.left) * (CANVAS_W / rect.width);
      const uy = (y - rect.top) * (CANVAS_H / rect.height);
  ...
  const handleCombatEvent = (lane: number, row: number, value: number, type: any) => {
    ...
    floatingLabels.current.push({
      ...
      type, life: 1.0, createdAt: Date.now()
    });
  };
  ...
  return (
    ...
    >
      ...
      <div className="p-2 px-4 border-b border-rhodes-border flex justify-between items-center bg-black/95 backdrop-blur-md z-30 shrink-0 shadow-lg">
        ...
        <div className="flex gap-4 sm:gap-8 items-center">
          <div className="flex flex-col items-center gap-1">
            <div className="flex items-center gap-2">
              <div className="flex gap-0.5">
                {[...Array(3)].map((_, i) => (
                  <div key={i} className={`h-4 w-1.5 rounded-sm skew-x-[-15deg] ${i < uiState.playerLP ? 'bg-rhodes-blue shadow-[0_0_5px_#19baff]' : 'bg-white/5 border border-white/5'}`} />
                ))}
              </div>
          ...
          <div className="flex flex-col items-center gap-1">
            <div className="flex items-center gap-2">
              ...
              <div className="flex gap-0.5">
                {[...Array(3)].map((_, i) => (
                  <div key={i} className={`h-4 w-1.5 rounded-sm skew-x-[-15deg] ${i < uiState.opponentLP ? 'bg-red-500 shadow-[0_0_5px_#ef4444]' : 'bg-white/5 border border-white/5'}`} />
                ))}
              </div>
            </div>
        ...
        <button onClick={() => setIsPaused(!isPaused)} className="p-2 rounded-full border border-white/10 text-white/40 hover:text-white">
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0 bg-black/98 z-[200] flex flex-col items-center justify-center p-4 backdrop-blur-md">
                ...
                <button onClick={() => {
                   if (mulliganSelected.length > 0) {
                      const newHand = [...playerHand]; const newDeck = [...playerDeck];
                      mulliganSelected.forEach(idx => {
                         const card = newHand[idx]; const next = newDeck.shift();
        ...
        <AnimatePresence>
          {winner && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="absolute inset-0 z-[1000] bg-black/90 flex flex-col items-center justify-center backdrop-blur-xl p-8 text-center">
                <h2 className={`text-6xl font-black terminal-text mb-4 tracking-tighter ${winner === 'PLAYER' ? 'text-rhodes-blue' : 'text-red-600'}`}>
                    {winner === 'PLAYER' ? 'VICTORY' : 'DEFEAT'}
                </h2>
                <p className="terminal-text text-[10px] text-white/50 mb-12 uppercase tracking-[0.4em]">Neural link resolution complete.</p>
      ...
      <div className="p-2 bg-[#050505] border-t border-rhodes-border shrink-0 z-20 shadow-2xl">
                ...
                <button 
                  onClick={() => {
                      if (playerHand.length < 6 && uiState.playerDP >= 5 && playerDeck.length > 0) {
                          ...
                          setPlayerHand([...playerHand, next]);
                          setPlayerDeck(playerDeck.slice(1));
                          if (isHost && kernelRef.current) { kernelRef.current.playerDP -= 5; syncMatchState(channel!); }
                      }
                  }}
        ...
        <div className="flex gap-2 overflow-x-auto pb-1 px-1 custom-scrollbar">
          ...
          {playerCooldowns.map((c, i) => (
             <div key={i} className="w-16 h-24 border border-white/5 bg-black/80 rounded-sm relative opacity-60 flex flex-col items-center justify-center shrink-0">
                <RotateCcw className="w-4 h-4 text-white/10 animate-spin-slow mb-1" />
                <span className="text-[8px] font-black text-rhodes-blue">{c.turnsRemaining}T</span>
             </div>
          ))}
        </div>
      </div>
